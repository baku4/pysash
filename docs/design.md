...
실현 밖 실행(residue) = `realized[r..]` + 이전에 밀려난 것들. 이들이 세션 상태에서
망가뜨렸을 수 있는 것의 상계를 잡는다.
...
statement 하나가 중간에 예외로 죽으면(`for p in paths: rows.append(load(p))`가 4번째에서 실패)
인터프리터에는 부분 효과가 남는데 세션은 그걸 표현할 수 없다. `poison()` 하나로 처리한다 —
이후 모든 정렬이 전면 Run을 낸다. 기존 경로를 재사용하므로 새 기계가 없다. **조용한 오염 대신
안전한 degrade.** 복구는 새 인터프리터 + 새 세션이다.
