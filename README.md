...
편집 루프는 `align → (Run만 실행) → realize`로 수렴한다. 실행이 중간에 끊겨 어디까지 돌았는지 모르면 그 소스를 `record_partial()`로 남긴다 — 그 소스가 건드렸을 수 있는 것만 오염되고 나머지 재사용은 살아남는다. 세션에 무슨 일이 있었는지 **아예** 모를 때만 `poison()`이다 — 이후는 전부 Run이며, 복구는 새 인터프리터 + 새 `SessionHistory`다.
